...
function initEndQuants(rows)
{
	for (var i = 0; i < rows.snapshotLength; i++)
	{
		...
		row.childNodes[END_QUANT_INDEX].firstChild.innerText =
			row.childNodes[START_QUANT_INDEX].firstChild.innerText;
	}
...
function attackOneWay(aRows, dRows)
{
	for (var i = 0; i < aRows.snapshotLength; i++)
	{
		...
		var aUnits = aRow.childNodes[START_QUANT_INDEX].firstChild.innerText - 0;
		var aPower = aRow.childNodes[POWER_INDEX].firstChild.nodeValue - 0;
		var aName = aRow.childNodes[NAME_INDEX].firstChild.innerText;
		...
		while (aUnits > 0.0001) // prevent spinning
		{
			...
			for (var j = 0; j < dRows.snapshotLength; j++)
			{
				...
				var dUnits = dRow.childNodes[END_QUANT_INDEX].firstChild.innerText - 0;
			...
			{
				var dRow = dRows.snapshotItem(j);
				var dName = dRow.childNodes[NAME_INDEX].firstChild.innerText;
				var dUnits = dRow.childNodes[END_QUANT_INDEX].firstChild.innerText - 0;
				if (dUnits == 0)
				{
				...
				if (damage >= dHp)
				{
					dRow.childNodes[END_QUANT_INDEX].firstChild.innerText = 0;
				...
				{
					dRow.childNodes[END_QUANT_INDEX].firstChild.innerText =
						(dHp - damage) / dArmor;
...
function roundEndQuants(rows)
{
	...
	{
		var row = rows.snapshotItem(i);
		var scale = findScale(row.childNodes[NAME_INDEX].firstChild.innerText);
		var roundedValue = roundUp(row.childNodes[END_QUANT_INDEX].firstChild.innerText, scale);
		row.childNodes[END_QUANT_INDEX].firstChild.innerText = roundedValue;

		if (row.childNodes[END_QUANT_INDEX].firstChild.innerText !=
				row.childNodes[START_QUANT_INDEX].firstChild.innerText)
		{
